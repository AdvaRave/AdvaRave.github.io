  ...
  install: function (Vue) {
    Vue.directive('smooth-scroll', {
      inserted: function (el, binding) {
        var targetElement = binding.arg;

        // Do not initialize smoothScroll when running server side, handle it in client
        // We do not want this script to be applied in browsers that do not support those
        // That means no smoothscroll on IE9 and below.
        if (typeof window !== 'object' || window.pageYOffset === void 0 || targetElement == null) return

        // Attach the smoothscroll function
        el.addEventListener('click', function (ev) {
          
          var scrollTo = document.getElementById(targetElement)
          if(!scrollTo) return
